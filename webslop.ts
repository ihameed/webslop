...
class wait_node extends Promise<void> {
    __wn_waitq : dlist = {...dummy_dlist}
    __wn_wake : () => void = null as any
}
...
export const wait_for_semaphore = (sem : semaphore) : Promise<void> => {
    if (sem.__semaphore_count == 0) {
        ...
        const ret = new wait_node((res, rej) => { resolve = res; void rej })
        ret.__wn_waitq.container = ret
        ret.__wn_waitq.next = ret.__wn_waitq
        ret.__wn_waitq.prev = ret.__wn_waitq
